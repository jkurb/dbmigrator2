	...
	public function write(OutputInterface $output, $table, $crop = null, $rowKeys = false)
	{
		...
		foreach ($table AS $row_key => $row)
		{
			...
			foreach ($row AS $key => $cell)
			{
				...
				$cell_count++;
				if (!isset($cell_lengths[$key]) || $cell_length > $cell_lengths[$key]) $cell_lengths[$key] = $crop === null ? $cell_length : min($cell_length, $crop);
			}
		...
		foreach ($table AS $key => $row)
		{
			...
			foreach ($row AS $key => $cell)
			{
				$out .= ' ' . str_pad(substr($cell, 0, $cell_lengths[$key]), $cell_lengths[$key], ' ', STR_PAD_RIGHT) . " |";
			}
			$output->writeln($out);
		}
